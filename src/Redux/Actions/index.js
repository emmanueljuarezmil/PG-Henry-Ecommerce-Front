...
export function getAllProducts(
  ...
  return async function (dispatch) {
    var json = await axios(
      `/products?page=${page}&name=${name}&orderBy=${orderBy}&orderType=${orderType}&category=${category}&descFilter=${descFilter}`
    );
...
export function getSearchBarProducts() {
  return async function (dispatch) {
    const { data } = await axios(`/productsforsearchbar`);
...
export const getProductDetail = (id) => {
  return async (dispatch) => {
    const { data } = await axios(`/products/p/${id}`);
    dispatch({
...
export const deleteFav = (id) => {
  return (dispatch) => {
    axios
      .delete(`/users/favs`, { data: { idProduct: id }, headers })
      .then((response) => {
        dispatch({
          type: DELETE_FAV,
          payload: id,
...
export const updateCategory = (body) => {
  return (dispatch) => {
    axios.put(`/category/update`, body, { headers }).then(() =>
      dispatch({
        type: null,
      })
    );
...
export const getAllUsers = (users, name, admin) => {
  if (users) {
    ...
    return async function (dispatch) {
      const users = await axios.get(`/users?name=${name}&admin=${admin}`, {
        headers,
      });
  ...
  if (!orders) {
    return async (dispatch) => {
      const { data } = await axios.get(
        `/orders?status=${order}&shippingStatus=${shipping}`,
        {
          headers,
        }
      );
...
export const getOrderDetail = (id) => {
  return async (dispatch) => {
    try {
      const { data } = await axios(`/orders/${id}`, { headers });
      dispatch({
...
export const localStorageCartToDB = (userId, headers) => async (dispatch) => {
  if (userId) {
    try {
      ...
      console.error(
        "removeStorage: Error removing key cart from localStorage: " +
          JSON.stringify(e)
      );
    }
  }
...
export const DBcartToLocalStorage = (idUser) => async (dispatch) => {
  try {
    const { data } = await axios(`/cart/${idUser}`, { headers });
    console.log(data);
...
export const deleteFromCart = (userId, idProduct) => async (dispatch) => {
  ...
  if (userId) {
    axios
      .delete(`/cart/${userId}/${idProduct}`, { headers })
      .then((res) => {
...
export const changeQuantity =
  (product, quantity, userId) => async (dispatch) => {
    if (userId) {
      axios
        .put(
          `/cart/${userId}`,
...
export const updateShippingAddress =
  (idUser, shippingAddress) => async (dispatch) => {
    if (idUser) {
      axios
        .put(`/users/updateShippingAddress/${idUser}`, { shippingAddress })
        .then((res) => {
...
export const getShippingAddress = (idUser) => async (dispatch) => {
  if (idUser) {
    axios
      .get(`/users/getShippingAddress/${idUser}`)
      .then((res) => {
...
export const authenticationCode = (idUser) => async (dispatch) => {
  // CUANDO EL USER INGRESA POR PRIMERA VEZ, SE EJECUTA SIEMPRE PARA LLENAR EL ESTADO DE REDUX, PERO EL PUT SOLO SE HACE LA PRIMERA VEZ.
  axios
    .get(`/users/authenticationByCode/${idUser}`)
    .then((res) => {
      if (res.data === true)
        return dispatch({ type: AUTHENTICATED_BY_CODE, payload: res.data });
      if (res.data === false) {
        const authenticationCode = Math.floor(Math.random() * 1000000);
        axios.put(
          `/users/authenticationCode/${idUser}?authenticationCode=${authenticationCode}`,
          null,
          { headers }
        );
      }
...
//   axios.get(`/users/authenticationByCode/${idUser}?authenticationCode=${authenticationCode}`)
//     .then(res => {
...
export const authenticationByCode = (data) => (dispatch) => {
  // CUANDO EL USUARIO TIENE EL CODIGO Y LO TIENE QUE VERIFICAR
...
export const getUserOrders = (idUser) => {
  return (dispatch) => {
    axios
      .get(`/orders/users/${idUser}`, { headers })
